	...
	for(int i = 0; i<numManifolds; i++)
	{
		...
		if(numContacts > 0)
		{
			...
			if(pbodyA && pbodyB)
			{
				...
				std::list<Module*>::iterator item2 = pbodyB->collision_listeners.begin();
				for (; item2 != pbodyB->collision_listeners.end(); ++item) {
					if ((*item2) != nullptr) {
...
bool ModulePhysics3D::CleanUp()
{
	...
	std::list<btDefaultMotionState*>::iterator item2 = motions.begin();
	for (; item2 != motions.end(); ++item2) {
		if ((*item2) != nullptr) {
			delete (*item2);
			(*item2) = nullptr;
		}
	...
	std::list<btCollisionShape*>::iterator item3 = shapes.begin();
	for (; item3 != shapes.end(); ++item3) {
		if ((*item3) != nullptr) {
			delete (*item3);
			(*item3) = nullptr;
		}
	...
	std::list<PhysBody3D*>::iterator item4 = bodies.begin();
	for (; item4 != bodies.end(); ++item4) {
		if ((*item4) != nullptr) {
			delete (*item4);
			(*item4) = nullptr;
		}
