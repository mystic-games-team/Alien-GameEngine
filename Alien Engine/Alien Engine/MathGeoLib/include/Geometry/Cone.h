...
/// A 3D cylinder 
class __declspec(dllexport) Cone
{
public:
	...
	/** @param endPoints Specifies the line segment of the capsule.
		...
		@see l, r. */
	Cone(const LineSegment &endPoints, float radius);

	/// Constructs a new capsule by explicitly specifying the member variables.
	/** This constructor is equivalent to calling Cone(LineSegment(bottomPoint, topPoint), radius), but provided
		here for conveniency.
		@see l, r. */
	Cone(const float3 &bottomPoint, const float3 &topPoint, float radius);

	/// Computes the distance of the two inner points of this capsule.
	/** <img src="ConeFunctions.png" />
		@see Height(). */
	...
	/// Returns the center point of this Cone.
	/** <img src="ConeFunctions.png" />
